...
def mesh_step_file(step_path: str, mesh_size: float, element_order: int,
    ...
    # First check whether STEP already contains valid solids.
    volumes = gmsh.model.getEntities(dim=3)

    print(f"[mesh] Initial 3-D volumes: {len(volumes)}")

    # Only attempt healing when the STEP does NOT already contain volumes.
    if not volumes:
        print("[mesh] No 3-D volumes found. Attempting OCC healing...")
        ...
        print(f"[mesh] 3-D volumes after healing: {len(volumes)}")

    if not volumes:
        gmsh.finalize()
        raise RuntimeError(
            "STEP contains no usable 3-D volume, even after healing."
        )
    ...
    gmsh.option.setNumber("Mesh.HighOrderOptimize", 0) # Untangle high-order elements
...
def main():
    ...
    p.add_argument("--mesh-size", type=float, default=2.0,
                   help="Target element edge length in mm  (default: 2.0)")
